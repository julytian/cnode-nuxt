  ...
  <div class="min-h-screen">
    ...
    <section class="flex flex-col items-center user">
      ...
      <div class="flex item-center justify-around w-full">
        <span v-if="user.create_at" v-text="getLastTimeStr(user.create_at, false)"></span>
        <span v-if="user.score" class="user__score">积分: {{ user.score }}</span>
      </div>
    </section>
    <van-tabs v-if="show" v-model:active="active" sticky>
      <van-tab title="最近回复">
        <van-empty v-if="user.recent_replies && user.recent_replies.length === 0" description="暂无数据" />
        <template v-else>
          <RecentItem v-for="item in user.recent_replies" :key="item.id" :item="item" />
        </template>
      </van-tab>
      <van-tab title="最新发布">
        <van-empty v-if="user.recent_topics && user.recent_topics.length === 0" description="暂无数据" />
        <template v-else>
          <RecentItem v-for="item in user.recent_topics" :key="item.id" :item="item" />
        </template>
      </van-tab>
      <van-tab title="用户收藏">
        <van-empty v-if="collects && collects.length === 0" description="暂无数据" />
        <template v-else>
